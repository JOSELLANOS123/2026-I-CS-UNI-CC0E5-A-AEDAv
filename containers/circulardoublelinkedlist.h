...
public:
    ...
    CDLLNode(T data, Ref ref, CDLLNode *next = nullptr, CDLLNode *prev = nullptr)
...
//  CDLLForwardIterator — avanza con getNext(), para al volver al inicio
...
public:
    ...
    CDLLForwardIterator(Container *pContainer, Node *pNode, bool)
...
//  CDLLBackwardIterator — avanza con getPrev(), para al volver al inicio
template <typename Container>
...
public:
    CDLLBackwardIterator(Container *pContainer, Node *pNode)
        : Parent(pContainer, pNode), m_start(pNode) {}
    CDLLBackwardIterator(Container *pContainer, Node *pNode, bool)
        : Parent(pContainer, pNode), m_start(nullptr) {}
...
//  Lista circular doblemente enlazada: m_tail->next = m_pRoot  y  m_pRoot->prev = m_tail siempre.
...
private:
    ...
    void internal_insert(const value_type &value, Ref ref) {
        ...
        m_size++;
        // Lista vacia: nodo se apunta a si mismo en ambas direcciones
        if (!m_pRoot) {
        ...
        // Buscar slot y enlazar en ambas direcciones
        Node *act = m_pRoot;
...
public:
    ...
    void clear() {
        ...
        if (!m_pRoot) return;
        m_tail->setNext(nullptr);   // romper circulo antes de liberar
        Node *curr = m_pRoot;
        while (curr) {
    ...
    // circularForEach: N vueltas en direccion +1 (fwd) o -1 (bwd)
    template <typename Func, typename... Args>
